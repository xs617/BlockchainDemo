...
//                    .hostnameVerifier { hostname, session -> true }
    ...
    suspend fun transaction(context: Context) {
        val retrofit = Retrofit.Builder()
            .baseUrl("https://mempool.space/testnet4/")
        ...
        val data = api.getAddressTransactions(myTapRoot)
        Log.e("wjr", "myAddress : $myTapRoot")
        Log.e("wjr", "otherAddress : ${otherTapRoot}")
        val result = data.awaitResponse()
        Log.e("wjr", "----- ${result.body()}")
        result.body()?.firstOrNull()?.let {
            val lastTransaction = Transaction(
                it.version ?: 2L,
                it.vin?.filterNotNull()?.map {
                    TxIn(
                        OutPoint(TxId(it.txid ?: ""), it.vout?.toLong() ?: 0L),
                        it.sequence ?: 0L
                    )
                } ?: listOf(), it.vout?.filterNotNull()?.map {
                    TxOut(
                        10000.toSatoshi(),
                        it.scriptpubkey?.toByteArray() ?: byteArrayOf()
                    )
                } ?: listOf(), it.locktime ?: 0L)
            // 创建一个 OutPoint，表示需要消费的 UTXO
            val outputScript =
                addressToPublicKeyScript(Block.Testnet4GenesisBlock.hash, otherTapRoot).right!!
            val tx1 = Transaction(
                2,
                listOf(TxIn(OutPoint(lastTransaction.txid, 1), TxIn.SEQUENCE_FINAL)),
                listOf(TxOut(Satoshi(10000), outputScript)),
                0
            )
            val sigHashType = 0
            val sig = Transaction.signInputTaprootKeyPath(
                ...
                tx1,
                0,
                listOf(lastTransaction.txOut[1]),
                sigHashType,
                scriptTree = null
            )

            val tx2 = tx1.updateWitness(0, fr.acinq.bitcoin.Script.witnessKeyPathPay2tr(sig))
            Log.e("wjr", "tx2 : $tx2")
...
//            val data = api.postTransaction(BigInteger(tx2.toString(),16))
//            val result = data.awaitResponse()
    ...
    suspend fun test2() {
        ...
        val total = response.body()?.fold(0L) { old, next ->
            old + next.value
        } ?: 0
        response.body()?.lastOrNull()?.let {
            val lastHex = api.getTransactionHex(it.txid).awaitResponse().body()
            Log.e("wjr", "lastHex : $lastHex")
            if (lastHex != null) {
                val lastTx = Transaction.read(lastHex)
                Log.e(
                    "wjr",
                    "script : ${fr.acinq.bitcoin.Script.pay2tr(mInternalKey, scripts = null)}"
                )
                Log.e(
                    "wjr",
                    "script2 : ${fr.acinq.bitcoin.Script.parse(lastTx.txOut.last().publicKeyScript)}"
                )
                val outputScript =
                    addressToPublicKeyScript(Block.Testnet4GenesisBlock.hash, otherTapRoot).right!!
                val myOutputScript =
                    addressToPublicKeyScript(Block.Testnet4GenesisBlock.hash, myTapRoot).right!!
                val fee = api.getRecommendedFees().awaitResponse()
                val inList = listOf(TxIn(OutPoint(lastTx, it.vout.toLong()), TxIn.SEQUENCE_FINAL))
                val testOutList = listOf(
                    TxOut(1000.sat(), outputScript),
                    TxOut(
                        (it.value - 1000L - (fee.body()?.fastestFee ?: 1L)).sat(),
                        myOutputScript
                    )
                )
                // we want to spend
                val calculateTxSize = Transaction(2, inList, testOutList, 0).totalSize()
                val realOutList = listOf(
                    TxOut(1000.sat(), outputScript),
                    TxOut(
                        (it.value - 1000L - (fee.body()?.fastestFee ?: 1L) * calculateTxSize).sat(),
                        myOutputScript
                    )
                )
                ...
                Log.e(
                    "wjr",
                    " base ${it.value} : fee: ${(fee.body()?.fastestFee ?: 1L) * calculateTxSize}"
                )
                val sigHashType = 0
                val sig = Transaction.signInputTaprootKeyPath(
                    mKey.privateKey,
                    ...
                    0,
                    lastTx.txOut.filter {
                        myTapRoot == addressFromPublicKeyScript(
                            Block.Testnet4GenesisBlock.hash,
                            it.publicKeyScript.toByteArray()
                        ).right
                    },
                    sigHashType,
                    scriptTree = null
                ...
                val signTx =
                    rawTx.updateWitness(0, fr.acinq.bitcoin.Script.witnessKeyPathPay2tr(sig))
                Transaction.correctlySpends(
                    signTx,
                    lastTx,
                    ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS
                )
                val data = api.postTransaction(
                    RequestBody.create(
                        "text/plain".toMediaType(),
                        signTx.toString()
                    )
                )
                Log.e("wjr", "send : $signTx")
                val result = data.awaitResponse()
                Log.e("wjr", "send result: $result")
            }
        }
